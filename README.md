...
We will now run the benchmarks using the multi process method. In this case, we will use a helper file (``jobs_multi.txt`` in this case) to run the ``executable.sh`` using different specifications for the compute capability. In the previous exercise, we defined ``(Capability > 6.0)`` which tells HTC to use any GPU with compute capability greater than 6.0 (basically any except the NVIDIA P100s). We would now like to run the benchmarks on specific GPUs by specififying compute capabilities (you can read more about this on CHTC's website). From the ``jobs_multi.txt`` we can see that we have four lines, each line containing a number for the compute capability requested. When we run the ``submit_multi.sub`` submit file, Condor will read each line as variables that are specified in the ``queue`` line. These can then be used throughout the rest of the submit file, hence we can change parameters from the submit file using the helper file.
